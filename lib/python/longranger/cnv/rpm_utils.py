...
class rPMRecord(object):
    def __init__(self, line=None):
        if line is None:
            ...
            self.Nread = self.ttlMol - self.ttlMolMs + self.NakeReads*0.5
            ...
            self.isHomoDel = self.ttlMol == self.ttlMolMs
...
class MergedBin (rPMRecord):
    def __init__(self, binList, mode="het_del", MolPerSZ=None):
        ...
        self.Nread = len(self.MIDAllSeen) + 0.5*self.NakeReads
        ...
        if mode=="het_del":
            ...
            if self.Nread == 0:
                print self.ttlMol, self.ttlMolMs, self.NakeReads, self.Nread
                self.isHomoDel= True
                self.HomoDelPVal = 1e-99
            elif self.exonID%10==5 and self.Nread <=2:
                ...
                if self.HomoDelPVal <= 1e-5: self.isHomoDel= True
