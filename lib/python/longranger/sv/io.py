...
def bedpe_to_vcf(bedpe_file, vcf_file, sample_name, source_str, reference_path):
    ...
    info_field_defs.append(vcf.parser._Info('POS', 1, 'Integer', 'End position of the variant described in this record', None, None))
    ...
    for _, row in bedpe_df.iterrows():
        ...
        if sv_type != 'DISTAL' and row.chrom1 == row.chrom2 and ps1 == ps2:
            ...
            #
            # the event starts at position pos (0-based)
            # so we'll write pos + 1 in the VCF (which is 1-based), and
            # fetch the (0-based) position pos - 1
            ref = get_fasta_base(fasta, row.chrom1, max(0, pos - 1))

            records = [vcf.model._Record(row.chrom1, pos + 1, row['name'],
