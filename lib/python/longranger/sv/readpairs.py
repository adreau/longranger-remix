    ...
    for start, stop in zip(starts, stops):
        for read in in_bam.fetch(str(chrom), start, stop):
            ...
            if (read.qname, chrom1, pos1) in tmp_pair_info or (read.qname, chrom1, pos1 - 1) in tmp_pair_info:
                ...
                good_dist = (chrom1 != chrom2 or np.abs(pos1 - pos2) > max(READ_MATE_CHIM_TOO_CLOSE_DIST, max(min_sv_len, max_insert))) and np.abs(pos1 - pos2) < max_sv_len
        ...
        if len(pos_pairs) > 0:
            ...
            if np.any(sel):
                ...
                for rp in type_readpairs:
                    ...
                    if overlaps(rp_range[0], support_range[0], 0) and overlaps(rp_range[1], support_range[1], 0):
...
class ReadPair(object):
    ...
    def __str__(self):
        return str(self.read1) + ',' + str(self.read2)
