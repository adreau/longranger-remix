...
def main(args, outs):
    ...
    if not re.match("^(disable|freebayes|gatk:/.*\.jar|precalled:/.*\.vcf)$", args.vc_mode):
        martian.exit("vc_mode must be of the form 'freebayes', 'gatk:/path/to/GenomeAnalysisTK.jar', 'disable'.")
...
def check_gatk(jar_path, hostname):
    ...
    if "3.3" not in output and "3.4" not in output and "3.5" not in output and "3.7" not in output:
        martian.exit("Version of GATK must be 3.3-3.5, version detected %s. Released version 3.6 has a breaking bug for haploid mode. More recent versions including nightly builds may work, but are not tested and you must disable preflight checks" % (output))
