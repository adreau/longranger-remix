...
def split(args):
    ...
    for key, position_chunk in sorted(pos_chunks.iteritems()):
        chunk_defs.append({'position_chunk':position_chunk, "__mem_gb": 5})
...
def main(args, outs):
    ...
    centromeres = load_centromere_file(centromere_file)
    print centromeres
    update_mapqs(outs.pos_sorted_bam+"tmp.bam", outs.pos_sorted_bam+"tmp2.bam", args.reference_path, centromeres)
    subprocess.check_call(['rm',outs.pos_sorted_bam+"tmp.bam"])
    subprocess.check_call(['samtools','sort', "-m", "3G", outs.pos_sorted_bam+"tmp2.bam", outs.pos_sorted_bam[:-4]])
