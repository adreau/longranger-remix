...
class BcSimilarity:
    ...
    def __init__(self, fragments_fn, bcs_to_use, bam_fn):
        ...
        bin_size = genome_length/GENOME_BINS
    ...
    def coalescence_analysis(self, min_cluster_size = 2, fpr = 1.0):
        ...
        # Construct barcode overlap graph
        ovl_graph = nx.Graph()
        n_overlaps = 0
        ...
        for x in bc_bins:
            ...
            for y in bc_bins:
                ...
                detected_overlaps = self.window_intersection_slices(x, y, bc_bin_size, pvalue_cut, overlap_threshold)

                for (bc1, bc2, overlaps, score) in detected_overlaps:
                    ovl_graph.add_edge(bc1, bc2, overlaps=overlaps, score=score)
                    n_overlaps += 1

        martian.log_info("Detected %d overlapping BCs" % n_overlaps)

        # Delete the fragment matrix to save memory
        del self.mat
        self.mat = None

        sgs = nx.connected_component_subgraphs(ovl_graph)
        bad_bcs = []
        sg_idxs = []
        cluster_size = []
        for (sg_idx, sg) in enumerate(sgs):
            ...
            sg_idxs.extend([sg_idx] * len(nodes))
            cluster_size.extend([len(nodes)] * len(nodes))

        bad_bc_tbl = p.DataFrame({'bc': bad_bcs, 'cluster_id': sg_idxs, 'cluster_size': cluster_size})
        return (bad_bc_tbl, ovl_graph)
    ...
    def window_intersection_slices(self, bc_offset1, bc_offset2, bc_bin_size, pvalue_threshold, overlap_threshold):
        ...
        for (elem_idx, (i1, i2)) in enumerate(zip(i1s, i2s)):
            ...
            if pvalue < pvalue_threshold:
                score = -10.0 * np.log10(pvalue)
                detection = (self.bcs_to_use[i1], self.bcs_to_use[i2], overlaps, score)
                detected_overlaps.append(detection)
