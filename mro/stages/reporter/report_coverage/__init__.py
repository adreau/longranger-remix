...
def split(args):
    ...
    if args.targets_file is None:
        ...
        target_regions = tk_io.get_target_regions(open(args.targets_file, 'r'))
    ...
    chunk_defs = [{'loci': loci, 'subsample_rate': subsample_rate} for loci in locus_sets]
...
def join(args, outs, chunk_defs, chunk_outs):
    ...
    if args.targets_file is not None:
        targets = tk_io.get_target_regions(open(args.targets_file))
    ...
    top_index = np.where(cum_sites >= num_sites.sum() * 0.995)[0][0]
    ...
    if cov_h5:
        in_files = [out.coverage for (cdef, out) in zip(chunk_defs, chunk_outs)]
    ...
    if args.targets_file and outs.target_coverage:
        in_files = [out.target_coverage for (cdef, out) in zip(chunk_defs, chunk_outs)]
    ...
    if args.targets_file:
        in_files = [out.coverage_off_target for (cdef, out) in zip(chunk_defs, chunk_outs)]
...
def main(args, outs):
    ...
    if args.targets_file is None:
        ...
        target_regions_genome = tk_io.get_target_regions(open(args.targets_file, 'r'))
    ...
    if args.confident_regions is None:
        ...
        conf_regions_genome = tk_io.get_target_regions(open(args.confident_regions))
...
def get_depth_info(read_iter, chrom, cstart, cend, targets, conf_regions, unambiguous, high_coverage_bed, high_cov_threshold, misc_regions):
    depths = np.zeros(cend-cstart, np.int32)
    mapqv = np.zeros(cend-cstart, np.uint32)
...
def get_bc_depth_info(fragment_iter, chrom, cstart, cend, targets, conf_regions, unambiguous):
