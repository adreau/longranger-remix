...
def split(args):
    ...
    if args.restrict_locus is None:
        loci = tk_chunks.chunk_by_locus(chroms, chrom_lengths, tk_chunks.get_parallel_locus_size(args.targets_file),
            contig_whitelist = primary_contigs, extra_args = {'__mem_gb': mem_gb, '__threads': threads, 'split_input': precalled_file})
    else:
...
def main(args, outs):
    ...
    if not (vc_mode == "precalled"):
        ...
        if bed_length > 0 and chrom in primary_contigs:
            if variant_caller == 'freebayes':
                with open(outs.default, 'w') as output_file:
                    tenkit.log_subprocess.check_call(['freebayes', '-f', fasta_path, '-b', args.input, '-0', '-t', bedfile], stdout=output_file)
            elif variant_caller == "gatk":
                mx_arg = "-Xmx%dG" % int(args.__mem_gb)
                tenkit.log_subprocess.check_call(['java', mx_arg, '-jar', gatk_path,'-T','HaplotypeCaller',
                            '-R',fasta_path,'-I',args.input,
                            '--genotyping_mode', 'DISCOVERY',
                            '-stand_emit_conf','10','-stand_call_conf','30',
                            '-L',bedfile,'-o',outs.default])
            else:
                raise NotSupportedException('Variant caller not supported: ' + vc_mode)
