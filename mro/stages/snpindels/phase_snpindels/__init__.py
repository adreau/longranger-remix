...
def split(args):
    ...
    if args.restrict_locus is None:
        loci = tk_bam.generate_tiling_windows(input_bam, tenkit.constants.PARALLEL_LOCUS_SIZE, overlap=args.chunk_stitching_overlap)
    else:
    ...
    (frac_het_snps, bcs_per_het_snp) = smooth_sample_bcs_and_het_snps(args.input, primary_contig_lengths)
    ...
    for (idx, locus) in enumerate(loci):
        ...
        if args.fragments is None or chrom not in primary_contigs or chrom in remove_chroms:
            ...
            est_gb = np.ceil(0.5 + 0.00015 * est_het_snps) # empirical memory usage
            min_gb = 6
            if np.isnan(est_gb):
                mem_gb = min_gb
            else:
...
def join(args, outs, chunk_defs, chunk_outs):
    ...
    # stitch phase blocks
    (stitched_chrom_vcfs, stitched_chrom_frags) = stitcher.multi_join_parallel(frag_files, vcf_files, loci)
...
def combine_frags(out_frags, in_frags_list):
    ...
    with open(out_frags, 'w') as fout:
        header = ['chrom', 'frag_start', 'frag_end', 'phase_set', 'ps_start', 'ps_end', 'bc', 'h0', 'h1', 'hmix', 'reads']
...
def sample_bcs_and_het_snps(vcf, contigs):
    ...
    for i in xrange(num_samples):
        ...
        het_snp_recs += snp
        het_snp_bcs += bcs
    return (tk_stats.robust_divide(het_snp_recs, total_recs), tk_stats.robust_divide(het_snp_bcs, het_snp_recs))
...
def smooth_sample_bcs_and_het_snps(vcf, contigs):
    ...
    for i in xrange(num_reps):
        (het_snps, bcs) = sample_bcs_and_het_snps(vcf, contigs)
        mean_bcs.append(bcs)
        mean_het_snps.append(het_snps)
    return (np.mean(mean_het_snps), np.mean(mean_bcs))
