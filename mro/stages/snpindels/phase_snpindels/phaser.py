...
class Phaser(object):
    ...
    def __init__(self, vfr, fragments, chrom, start, stop, bc_mix_prob=0.0005, min_junction_hap_conf=0.999, min_var_hap_conf=0.999, block_buffer_size=2, hap_block_size=10, max_reassign_rounds=2, vc_mode = "call"):
        ...
        (sub_bc_indexes, bc_regions, self.contig_starts, self.contig_stops, self.contig_bcs, self.num_reads) = self.get_bc_contig_info(chrom, start, stop, fragments)
    ...
    def get_bc_contig_info(self, chrom, start, stop, fragments):
        ...
        if fragments is not None:
            ...
            chunk_frags = tenkit.hdf5.read_data_frame_indexed(fragments, query, query_cols=['chrom', 'start_pos', 'end_pos', 'bc', 'num_reads'])
        ...
        return (sub_bc_indexes, bc_regions, np.array(starts, dtype=int), np.array(stops, dtype=int), bcs, np.array(reads,dtype=int))
    ...
    def get_bc_variant_matrices(self, num_bcs, num_poses, bc_regions, sub_bc_indexes):
        ...
        for record in tk_io.get_variant_iterator_pos(self.vfr, None, tk_io.create_locus_info(self.chrom, self.start, self.stop)):
            ...
            for bc in bcs_allele1:
                ...
                bc_quals1[bc_seq] = [math.log(math.pow(10.0, -(int(bc_qual) - ILLUMINA_QUAL_OFFSET)/10.0)) for bc_qual in bc_quals]
            ...
            for bc in bcs_allele2:
                ...
                bc_quals2[bc_seq] = [math.log(math.pow(10.0, -(int(bc_qual) - ILLUMINA_QUAL_OFFSET)/10.0)) for bc_qual in bc_quals]
            # Double-check below
            seen_bcs = set()
    ...
    def write_fragment_phasing(self, overall_hap_assign, phase_block_ids, phased_vars, fragment_phasing):
        ...
        for i in range(num_blocks):
            ...
            for (f_start, bc_ind, chunk_idx) in sorted_bc_inds:
                ...
                line = [self.chrom, frag_start, frag_end, phase_block_name, ps_start, ps_end, bc, h1, h2, hm, frag_reads]
