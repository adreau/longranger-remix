...
def main(args, outs):
    ...
    if len(pred_df) > 0:
        ...
        blacklist_file = None
        if args.sv_blacklist_regions is not None:
            blacklist_file = args.sv_blacklist_regions

        elif os.path.exists(tk_reference.get_sv_blacklist(args.reference_path)):
            blacklist_file = tk_reference.get_sv_blacklist(args.reference_path)

        else:
            genome = tk_reference.get_genome(args.reference_path)
            builtin_blacklist = tenkit.constants.find_sv_blacklist(genome)
            if builtin_blacklist is not None:
                blacklist_file = builtin_blacklist
        ...
        seg_dups_file = None
        if args.seg_dups is not None:
            seg_dups_file = args.seg_dups

        elif os.path.exists(tk_reference.get_segdups(args.reference_path)):
            seg_dups_file = tk_reference.get_segdups(args.reference_path)

        else:
            genome = tk_reference.get_genome(args.reference_path)
            builtin_segdups = tenkit.constants.find_segdups(genome)
            if builtin_segdups is not None:
                seg_dups_file = builtin_segdups
