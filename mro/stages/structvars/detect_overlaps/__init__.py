...
def split(args):
    ...
    if nloci > 1:
        ...
        nchunks = min(max(10, 500 - nloci), len(chunk_combs))
...
def main(args, outs):
    ...
    if test == tk_sv_utils.EXP_COUNT2:
        fragments = tk_hdf5.read_data_frame(args.fragments, query_cols = ['bc', 'obs_len'])
        ...
        fragments = tk_hdf5.read_data_frame(args.fragments, query_cols = ['bc', 'obs_len', 'bc_num_reads', 'num_reads'])
        # Remove short fragments, these don't affect the statistic much at all
        fragments = fragments[np.logical_and(fragments.num_reads > 1, fragments.bc_num_reads > 2)]
        fragments.drop(['bc_num_reads', 'num_reads'], axis=1, inplace=True)
        if len(fragments) < MIN_NUM_FRAGS:
            martian.log_info('Too few fragments ({}). No calls will be made.'.format(len(fragments)))
            with open(outs.overlap_loci, 'wb') as f:
                cPickle.dump(overlap_loci, f)
